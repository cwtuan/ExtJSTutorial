					...
					items : [ {
						...
						listeners : {
							afterRender : function(thisForm, options) {
								...
								if ('${model.valid}' === 'true') {
									this.down('#message').setMessage(true, Locale.getMsg('view.auth.email.activate.redirect', '${model.email}', '3'));
									setTimeout("window.location = './';", 3000);
								} else {
									this.down('#message').setMessage(false, Locale.getMsg('view.auth.email.activate.duplicated', '${model.email}', '3'));
									setTimeout("window.location = './';", 3000);
								}
								if ('${model.error}' != '') {
									var temp = new Object();
