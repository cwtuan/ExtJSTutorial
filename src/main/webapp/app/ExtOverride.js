...
Ext.define('Ecfa.ExtOverride', {
	...
	init : function(vosConfig) {
		...
		// TODO upload using new HTML5 File API
		// Ext.define('Ext.enhance.Ajax', {
		// override : 'Ext.data.Connection',
		// upload: function(form, url, params, options) {
		// this.callParent([form, url, params, options]);
		// }
		// });
		...
		Ext.define('Ext.enhance.grid.Panel', {
			override : 'Ext.grid.Panel',
			emptyText : '<div style="font-size: medium;text-align:center">' + Locale.getMsg('view.common.noRecords') + '</div>',
			initComponent : function() {
				var me = this;
				me.callParent(arguments);
				...
				//
				// var proxy = me.store.getProxy(), reader = proxy.getReader(), raw = reader.rawData;
				//
				// console.log(reader.getMessage(raw));
				//
				// me.getView().emptyText = '<div align="center">' + Locale.getMsg('err.'+reader.getMessage(raw)) + '</div>';
				// me.getView().refresh();
				...
				me.store.on({
					load : function(store, records, successful) {
						// No records found
						if (successful) {
							me.getView().emptyText = '<div style="font-size: medium;text-align:center">' + Locale.getMsg('view.common.noRecords') + '</div>';
						...
						// Fail to load
							...
							if (me.id) {
								...
								console.log('failed to load grid', reader.type, store.getProxy().url);
								...
								if (reader.type === 'restTaskGrid') {
									...
									if (Locale.hasKey(key)) {
										me.getView().emptyText = '<div style="font-size: medium;color:red;text-align:center">' + '<p>'
												+ Locale.getMsg('view.common.load.error') + '</p><p>' + Locale.getMsg('err.reason') + Locale.getMsg(key)
												+ '</p>' + '</div>';
									}
									// internal error
									else {
								...
								// for store with normal (rest) reader. It's assumed all errors are internal errors
								else {
									me.getView().emptyText = '<div style="font-size: medium;color:red;text-align:center">'
											+ Locale.getMsg('view.common.load.error.internal', "grid-reload-" + me.id) + '</div>';
